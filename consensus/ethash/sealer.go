...
type ParamsSubmitWorkToMain struct{
	Hash common.Hash `json:"hash"`
	Difficulty  *big.Int	`json:"difficulty"`
}

func (ethash *Ethash) SubmitWorkToMain(head *ParamsSubmitWorkToMain) {
	request := map[string]interface{}{
		"id":      rand.Int(),
		"method":  "eth_acceptAuxWork",
		"version": "2.0",
		"params":  head,
	}
	bs, err := json.Marshal(request)
	if err == nil {
		ethash.config.Log.Error("SubmitWorkToMain json Marshal:", err)
		return
	}
	requestBody := bytes.NewBuffer(bs)

	bodyType := "application/json;charset=utf-8"
	host := fmt.Sprintf("%s:%d", ethash.config.MainChainRPCHost, ethash.config.MainChainRPCPort)
	resp, err := http.Post(host, bodyType, requestBody)
	if err != nil {
		ethash.config.Log.Error("SubmitWorkToMain Post:", err)
		return
	}
	if resp.StatusCode != http.StatusOK {
		ethash.config.Log.Error("SubmitWorkToMain resp.StatusCode=", resp.StatusCode)
		return
	}
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		ethash.config.Log.Error("SubmitWorkToMain ReadAll:", err)
		return
	}

	ethash.config.Log.Info("SubmitWorkToMain Response", body)
