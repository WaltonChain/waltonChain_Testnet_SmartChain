...
func (ethash *Ethash) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, fulldag bool) error {
	...
	digest, result := myx11(ethash.SealHash(header).Bytes(), header.SealNonce().Uint64(), order)
...
func (ethash *Ethash) SealHash(header *types.Header) (hash common.Hash) {
	...
	var enc []interface{}
	if header.AuxNumber.Sign() != 0 {
		enc = []interface{}{
			header.AuxParentHash,
			header.AuxUncleHash,
			header.AuxCoinbase,
			header.AuxRoot,
			header.AuxTxHash,
			header.AuxReceiptHash,
			header.AuxBloom,
			header.AuxDifficulty,
			header.AuxNumber,
			header.AuxGasLimit,
			header.AuxGasUsed,
			header.AuxTime,
